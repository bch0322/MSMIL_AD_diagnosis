...
def train(config, fold, model, model_1, optimizer, scheduler, list_dir_save_model, dir_pyplot, Validation=True, Test_flag = True):
    ...
    ##TODO : load masking img

    mask_img = torch.tensor(nib.load(st.dir_mask_img).get_data().squeeze()).cuda()
    ...
    for epoch in range(config.num_epochs):
        ...
        if Validation == True:
            print("------------------  val  --------------------------")
            dict_result = ut.eval_classification_using_pretrained_with_mask(config, fold, val_loader, model, model_1, criterion_cls, mask_img)
        ...
        if fst.flag_print_trainAcc== True:
            ...
            """ eval """
            dict_result = ut.eval_classification_using_pretrained_with_mask(config, fold, train_loader, model, model_1, criterion_cls, mask_img)
            acc = dict_result['Acc']
        ...
        if Test_flag== True:
            ...
            """ load data """
            dict_result = ut.eval_classification_using_pretrained_with_mask(config, fold, test_loader, model, model_1, criterion_cls, mask_img)
            acc = dict_result['Acc']
