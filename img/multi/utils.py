...
def eval_classification_model(config, fold, loader, model, criterion, confusion_save_info = None):
    ...
    with torch.no_grad():
        ...
        if plotting_flag == True:
            ...
            count_class_0 = 0
            count_class_1 = 0
            list_evi_class_0 = [[] for i in range(3)]
            list_evi_class_1 = [[] for i in range(3)]

            list_group_0 = [[] for i in range(3)]
            list_group_1 = [[] for i in range(3)]
            # list_evi_class_0 = []
            # list_evi_class_1 = []
            for datas, labels, alabels, mlabel in loader:
                ...
                if labels == 0:
                    list_group_0[0].append(dict_result['final_evidence_a'][:, 1].data.cpu().numpy())
                    list_group_0[1].append(dict_result['final_evidence_b'][:, 1].data.cpu().numpy())
                    list_group_0[2].append(dict_result['final_evidence_c'][:, 1].data.cpu().numpy())
                elif labels == 1:
                    list_group_1[0].append(dict_result['final_evidence_a'][:, 1].data.cpu().numpy())
                    list_group_1[1].append(dict_result['final_evidence_b'][:, 1].data.cpu().numpy())
                    list_group_1[2].append(dict_result['final_evidence_c'][:, 1].data.cpu().numpy())
                if labels == 0 and pred == 0 and count_class_0 < n_sample_each:
                    count_class_0 +=1
                    list_evi_class_0[0].append(dict_result['final_evidence_a'][:, 1].data.cpu().numpy())
                    list_evi_class_0[1].append(dict_result['final_evidence_b'][:, 1].data.cpu().numpy())
                    list_evi_class_0[2].append(dict_result['final_evidence_c'][:, 1].data.cpu().numpy())
                elif labels == 1 and pred == 1 and count_class_1 < n_sample_each:
                    count_class_1 += 1
                    list_evi_class_1[0].append(dict_result['final_evidence_a'][:, 1].data.cpu().numpy())
                    list_evi_class_1[1].append(dict_result['final_evidence_b'][:, 1].data.cpu().numpy())
                    list_evi_class_1[2].append(dict_result['final_evidence_c'][:, 1].data.cpu().numpy())
